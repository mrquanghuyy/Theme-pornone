...
add_action('init', function() {
    ...
    $slug_pornstars = get_option('ophim_slug_pornstars') ?: 'pornstars';
    $slug_pornstars = trim($slug_pornstars, '/');
    add_rewrite_rule('^' . preg_quote($slug_pornstars, '/') . '/?$', 'index.php?actors_list=1', 'top');
    add_rewrite_rule('^' . preg_quote($slug_pornstars, '/') . '/page/([0-9]+)/?$', 'index.php?actors_list=1&paged=$matches[1]', 'top');
